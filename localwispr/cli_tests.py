"""CLI test commands for LocalWispr.

This module contains the test commands that can be run from the CLI:
- record_test: Test audio recording functionality
- transcribe_test: Test transcription functionality
- context_test: Test context detection functionality
- hotkey_test: Test hotkey-triggered recording and transcription

These functions are invoked via subcommands like `localwispr record-test`.
"""
...
def transcribe_test(model_name: str | None = None) -> int:
    ...
    try:
        ...
        if result.inference_time < 2.0:
            ...
            print(f"Latency Target:  {result.inference_time:.2f}s (target: <2s)")
...
def hotkey_test() -> int:
    ...
    # Reference to listener for callbacks
...
__all__ = ["record_test", "transcribe_test", "context_test", "hotkey_test"]
